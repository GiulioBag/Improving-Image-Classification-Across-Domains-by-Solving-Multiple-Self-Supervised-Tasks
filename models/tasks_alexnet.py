...
class Tasks_Alexnet(nn.Module):
    def __init__(self, n_classes=100, jigen_classes=30, odd_classes = 9, dropout=True):
        super(Tasks_Alexnet, self).__init__()
        print("Using  AlexNet")
        ...
        self.jigen_classifier = nn.Linear(4096, jigen_classes + 1)
        self.rotation_classifier = nn.Linear(4096, 4)
        self.odd_one_classifier = nn.Linear(4096, odd_classes)

    def get_params(self, base_lr):
        return [{"params": self.features.parameters(), "lr": 0.},
                {"params": chain(self.classifier.parameters(), self.class_classifier.parameters(), self.jigen_classifier.parameters(), self.rotation_classifier.parameters(), self.odd_classifier.parameters()), "lr": base_lr}]
    ...
    def forward(self, x, lambda_val=0):
        ...
        x = x.view(x.size(0), -1)
        x = self.classifier(x)
        return self.class_classifier(x), self.jigen_classifier(x), self.rotation_classifier(x), self.odd_one_classifier(x)

def tasks_Alexnet(classes, jigen_classes, odd_classes):
    model = Tasks_Alexnet(classes, jigen_classes, odd_classes)
